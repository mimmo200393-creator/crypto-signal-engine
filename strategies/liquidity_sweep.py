...
LONG:
  - Livello: Pivot Low ultimi 50 H1, >= 2 tocchi, cluster <= 0.5 ATR
  - Sweep:   candle[-2] low < livello ma close > livello
  - Rejection: wick inferiore >= 2 * corpo reale
  - Trigger: candle[-1] close > candle[-2] high
  - SL = 1.5 x ATR, TP = swing significativo, R/R >= 2
SHORT: speculare (sweep di resistenza)
"""
...
class LiquiditySweep(BaseStrategy):
    ...
    def _check_sweep(self, candle, lp: float, direction: str) -> bool:
    ...
    def _score(self, sr: bool, sweep: bool, rejection: bool, trigger: bool) -> int:
        score = 0
        if sr:        score += 3
        if sweep:     score += 3
