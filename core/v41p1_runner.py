...
# Soglia di prossimita' per Watchlist Alert Phase 1
WATCHLIST_PROXIMITY_PCT = 0.005   # 0.5%
...
def _check_watchlist(conn, asset: str, mfm: dict, now: datetime, config: dict):
    ...
    current_price = mfm["current_price"]

    # Filtra livelli entro la soglia di prossimita'
    proximities = [
...
def _enrich_signal_with_mfm(signal: dict, mfm: dict) -> dict:
    """
    Arricchisce il dizionario signal con i dati della Money Flow Map:
    ...
    # Liquidity Source: livello di liquidita' da cui e' partito il trigger
    # (piu' vicino nella direzione OPPOSTA al trade — dove il prezzo era prima)
    if direction == "BUY":
        source_candidates = [
            lv for lv in mfm["levels"]
    ...
    # Source: il livello piu' vicino all'entry (da cui il mercato e' rimbalzato)
    liq_source = None
    if source_candidates:
        liq_source = min(source_candidates, key=lambda lv: abs(lv["price"] - entry))

    # Target: il livello con Priority Score piu' alto nella direzione del trade
    liq_target = None
    ...
    # Expected Move: distanza fino al target di liquidita'
...
def _run_for_asset(conn, asset: str, config: dict, macro_provider, now: datetime):
    ...
    logger.info(
        ...
        "source=%s target=%s em=%.1fpt (id=%s)",
        ...
        signal.get("expected_move_points") or 0,
        signal_id,
    )
